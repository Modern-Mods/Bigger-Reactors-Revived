...
public class CyaniteReprocessorCategory implements IRecipeCategory<CyaniteReprocessorCategory.Recipe> {
    ...
    public static final Identifier UID = Identifier.fromNamespaceAndPath(BiggerReactors.modid, "classic/cyanite_reprocessor");
    ...
    public void draw(Recipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
        ...
        for (int i = 0; i < info.length; i++) {
            guiGraphics.text(mc.font, info[i], 80 - mc.font.width(info[i]) / 2, i * 12, Color.BLACK.getRGB(), false);
        }
    }
